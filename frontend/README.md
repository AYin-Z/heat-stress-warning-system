...
| **空壳区分** | 区分 MQTT上线无数据（等待首次数据）vs HTTP注册空壳，持续监测自动切换 |
...
| 等级 | 条件 | 颜色 |
|------|------|------|
| 正常 | coreTemp < 38℃ | 🟢 绿 `#52c41a` |
| 普通预警 | 38℃ ≤ coreTemp < 39℃ | 🟠 橙 `#fa8c16` |
| 高风险预警 | coreTemp ≥ 39℃ | 🔴 红 `#ff4d4f` |
| 监测中 | 有数据但缺 coreTemp | 🔵 蓝 `#1890FF` |
| 数据不可用 | 未佩戴/无体征 | ◯ 灰 `#8c8c8c` |
| 离线 | 曾有健康数据但 90s 无上报 | ● 深灰 `#595959` |
| 等待首次数据 | MQTT已连接但从未上报健康数据 | 🟣 紫 `#722ed1` |
