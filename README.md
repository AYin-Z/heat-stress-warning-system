...
| 节点 | 角色 | 技术 | 部署 |
|------|------|------|------|
| 手表端 | 数据采集 | A80 Android (Kotlin) | A80 手表 |
| 中继服务器 | 消息分发+中介 | EMQX + bridge.py | 39.105.86.77 |
| 预测模型 | HR→核心温度 | FastAPI + PyTorch(Informer) | 20.205.12.160:8001 |
| 前端大屏 | 可视化+管理 | Django 5.2 + ECharts | 任意 Linux 服务器 |

**MQTT 主题**

| 主题 | 方向 | 频率 | 说明 |
|------|------|:----:|------|
| `watch/{id}/vital` | 手表→中继→大屏 | 15s | 心率/血氧/血压/GPS/步数/电量 |
| `watch/{id}/status` | 手表→中继→大屏 | 60s | 在线状态 + LWT 离线遗嘱 |
| `watch/{id}/bind` | 手表→中继 | 按需 | 设备绑定请求 |
| `watch/{id}/alert` | 中继→手表&大屏 | 按需 | 热应激预警（含核心温度） |
| `watch/{id}/core-temp` | 中继→大屏 | 按需 | 模型推算的核心温度 |
| `watch/{id}/bind/response` | 中继→手表 | 按需 | 绑定结果回执 |
| `watch/{id}/time` | 中继→手表 | 上线时 | 校时 |
